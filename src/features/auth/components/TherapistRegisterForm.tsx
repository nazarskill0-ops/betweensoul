...
export function TherapistRegisterForm() {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<TherapistRegisterValues>({
    resolver: zodResolver(therapistRegisterSchema),
    defaultValues: { consent: false, acceptVerification: false },
  });

  const mutation = useMutation({
    mutationFn: (v: TherapistRegisterValues) => signUp(v, "psychologist"),
    onSuccess: (res) => {
      if (res.needsEmailConfirm) return;
    ...
    return (
      <div className="w-full rounded-[20px] bg-white p-8 text-center shadow-sm">
        ...
        <p className="text-sm text-ink-muted">
          Ми надіслали лист для підтвердження на вашу пошту. Після цього наша
          команда зв&apos;яжеться з вами щодо верифікації.
        </p>
      </div>
    );
  }
      ...
      <div className="mt-4">
        <label className="flex items-start gap-2.5 text-sm text-ink">
          ...
          <span>
            ...
            перед публікацією профілю
          </span>
        </label>
        {errors.acceptVerification && (
          <span className={errorClass}>
            {errors.acceptVerification.message}
          </span>
        )}
      </div>
